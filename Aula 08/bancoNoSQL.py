...
lbl_codigo = Label(tela, text="Codigo", bg="#ffffff").place(x=130,y=140)
...
def salvar():
    ...
    cliente = {"codigo":codigo, "nome":nome, "idade":idade, "endereco":end, "cpf":cpf, "bairro":bairro,
...
def atualizar():
    ...
    collection.update_one({"codigo":codigo}, {"$set": {"codigo":codigo, "nome":nome, "idade":idade, "endereco":end, 
...
btn_consultar = Button(tela, text="Consultar", width=10, command=consultar).place(x=400, y=280)
btn_sair = Button(tela, text="Sair", width=10, command=tela.quit).place(x=490, y=280)
tela.mainloop()